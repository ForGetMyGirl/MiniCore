...
namespace MiniCore.Model
{
    ...
    public class NetworkSession : IClientSession
    {
        ...
        /// <param name="transport">会话持有的底层传输实现。</param>
        /// <param name="outboundExecutor">会话发送循环使用的执行器；为空时原生环境选择共享线程池，浏览器选择主循环。</param>
        public NetworkSession(string sessionId, INetworkTransport transport, IMTaskExecutor outboundExecutor = null)
        {
            SessionId = sessionId ?? throw new ArgumentNullException(nameof(sessionId));
            Transport = transport ?? throw new ArgumentNullException(nameof(transport));
            outboundQueue = new NetworkOutboundQueue(transport, outboundExecutor);
