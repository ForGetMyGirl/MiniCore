...
namespace MiniCore.Model
{
    ...
    public sealed class TcpServer : IDisposable
    {
        ...
        private readonly IMTaskExecutor networkExecutor; // 接入循环使用的执行器。

        /// <summary>
        /// 创建使用指定执行器的 TCP 监听器。
        /// </summary>
        /// <param name="executor">接入循环使用的执行器；为空时按当前环境选择默认执行器。</param>
        public TcpServer(IMTaskExecutor executor = null)
        {
