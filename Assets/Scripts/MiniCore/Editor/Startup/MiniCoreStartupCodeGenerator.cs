...
namespace MiniCore.EditorTools
{
    ...
    internal static class MiniCoreStartupCodeGenerator
    {
        ...
        /// <summary>
        /// 生成并写入所有项目运行形态共用的 HotUpdate 启动代码。
        /// </summary>
        ...
        private static string BuildGeneratedContent(MiniCoreStartupSettings settings, Dictionary<string, StartupModuleInfo> moduleByName, List<AppServiceInfo> services, List<AppModuleInfo> appModules)
        {
            ...
            builder.AppendLine();
            builder.AppendLine("        /// <summary>");
            builder.AppendLine("        /// 初始化项目中已启用的模块，并执行项目 GameStartup。");
            builder.AppendLine("        /// </summary>");
            ...
            builder.AppendLine();
            AppendStartupMethod(builder, settings, moduleByName, services);
            builder.AppendLine();
            builder.AppendLine("        #endregion");
            builder.AppendLine("    }");
            builder.AppendLine("}");
        ...
        /// <param name="settings">项目启动配置。</param>
        /// <param name="moduleByName">程序集限定类型名到发现模块的映射。</param>
        private static void AppendStartupMethod(StringBuilder builder, MiniCoreStartupSettings settings, Dictionary<string, StartupModuleInfo> moduleByName, List<AppServiceInfo> services)
        {
            List<StartupModuleInfo> orderedModules = ResolveOrderedModules(settings, moduleByName);
            bool hasAsyncService = HasEnabledAsyncAppService(settings, services);
            builder.AppendLine("        /// <summary>");
            builder.AppendLine("        /// 初始化项目中已启用的全局模块。");
            builder.AppendLine("        /// </summary>");
            builder.AppendLine("        /// <returns>项目启动完成任务。</returns>");
            builder.AppendLine($"        private static {(hasAsyncService ? "async " : string.Empty)}MTask StartConfiguredAsync()");
            builder.AppendLine("        {");
            AppendAppServiceStatements(builder, settings, services);
        ...
        /// <summary>
        /// 判断项目中启用的服务是否包含需要异步初始化的实现。
        /// </summary>
        /// <param name="settings">项目启动配置。</param>
        /// <param name="services">已发现 AppService 实现。</param>
        /// <returns>存在异步初始化服务时返回 true。</returns>
        private static bool HasEnabledAsyncAppService(MiniCoreStartupSettings settings, List<AppServiceInfo> services)
        {
            for (int index = 0; index < settings.Services.Count; index++)
            {
        ...
        /// <summary>
        /// 写入项目统一的接口绑定、服务启动和异步初始化语句。
        /// </summary>
        /// <param name="builder">目标代码构建器。</param>
        /// <param name="settings">项目启动配置。</param>
        /// <param name="services">已发现 AppService 实现。</param>
        private static void AppendAppServiceStatements(StringBuilder builder, MiniCoreStartupSettings settings, List<AppServiceInfo> services)
        {
            ...
            for (int index = 0; index < ordered.Count; index++)
            {
                ...
                for (int contractIndex = 0; contractIndex < contracts.Length; contractIndex++)
                {
                    ...
                    if (contracts.Any(contract => contract == typeof(INetworkService)))
                    {
        ...
        private static void VisitService(AppServiceInfo provider, Dictionary<Type, AppServiceInfo> selectedByContract, HashSet<Type> visited, HashSet<Type> visiting, List<AppServiceInfo> result)
        {
            ...
            foreach (Type dependency in (provider.Attribute.RequiresServices ?? Array.Empty<Type>()).OrderBy(type => type.FullName, StringComparer.Ordinal))
            {
                ...
                {
                    throw new InvalidOperationException($"AppService {provider.Type.FullName} 依赖的服务接口 {dependency.FullName} 未在项目启动配置中启用 Provider。");
                }
        ...
        /// <summary>
        /// 根据勾选状态和 DependsOn 解析项目统一的稳定依赖顺序。
        /// </summary>
        /// <param name="settings">项目启动配置。</param>
        /// <param name="moduleByName">程序集限定类型名到发现模块的映射。</param>
        /// <returns>依赖在前的启动模块顺序。</returns>
        private static List<StartupModuleInfo> ResolveOrderedModules(MiniCoreStartupSettings settings, Dictionary<string, StartupModuleInfo> moduleByName)
        {
