...
namespace MiniCore.Demo.MiniBomber
{
    ...
    public sealed class MiniBomberClientStartupComponent : MiniBomberStartupComponentBase
    {
        ...
        private MiniBomberClientNetworkProfile networkProfile; // MiniBomber 客户端独有网络配置。
        ...
        public async MTask InitializeAsync()
        {
            ...
            networkProfile = await Resources.PreloadAssetAsync<MiniBomberClientNetworkProfile>(MiniBomberConstants.ClientNetworkProfileAddress);
            if (networkProfile == null)
            {
                throw new System.InvalidOperationException("缺少 MiniBomberClientNetworkProfile，请运行 MiniCore/Demos/MiniBomber/Create Default Assets。");
            }

            if (!networkProfile.EnableNetwork)
            {
                return;
            }

            accountSession = Global.GetOrAdd<AccountSessionComponent>(this);
            await accountSession.InitializeAsync(networkProfile);
        ...
        protected override void OnDispose()
        {
            ...
            if (networkProfile != null)
            {
                Resources?.ReleaseAsset(MiniBomberConstants.ClientNetworkProfileAddress);
            }
