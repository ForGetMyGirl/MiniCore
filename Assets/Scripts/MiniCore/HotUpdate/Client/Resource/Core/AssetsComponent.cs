...
namespace MiniCore.Core
{
    /// <summary>
    /// 资产管理组件。
    /// 依赖标签组件与 YooAsset 资源组件，并在自身 Awake 中获取二者，调用方不再需要额外执行资源注册。
    /// </summary>
    [MiniCoreStartupModule(
        "资产管理",
        DependsOn = new[] { typeof(TagsComponent), typeof(YooAssetResourceComponent) })]
    public class AssetsComponent : AComponent
    {

        private TagsComponent tagsComponent; // 场景标签组件。
        private IResourcesComponent resourcesComponent; // 资源加载组件。
        private IResourcesComponent ResourcesComponent
        {
            get
            {
                ...
                {
                    throw new System.InvalidOperationException("资源加载组件尚未初始化，请确认 YooAsset 资源模块已先启动。");
                }
        ...
        /// <summary>
        /// 获取资产管理所需的标签与资源组件依赖。
        /// 依赖组件由启动模块特性的 DependsOn 保证已经 Pin。
        /// </summary>
        public override void Awake()
        {
            tagsComponent = Global.Get<TagsComponent>(this);
            resourcesComponent = Global.Get<YooAssetResourceComponent>(this);
        }
