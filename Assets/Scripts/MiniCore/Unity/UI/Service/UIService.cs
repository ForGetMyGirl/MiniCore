...
{
    /// <summary>
    /// 基于业务注入注册表、WindowSession 和资源租约的项目 UI 应用服务。
    /// </summary>
    [AppService(
        "UI 框架",
    ...
    public sealed class UIService : AAppService, MiniCore.UI.IUIService, IAsyncAppService, IUIWindowSessionHost
    {
        ...
        private UIWindowRegistry registry; // 当前业务启动周期注入的窗口注册表。
        ...
        public override void Awake(ComponentInitArgs args)
        {
            ...
            registry = UIWindowRegistry.Project;
        }
        ...
        {
            return OpenCoreAsync(Registry.Get(routeName), null, null);
        }
        ...
        public async MTask InitializeAsync()
        {
            ...
            if (Registry.Count == 0)
            {
                throw new InvalidOperationException("项目尚未注册任何 UI 窗口，请先执行生成的 ProjectUIWindowRegistration。");
            }
        ...
        {
            return OpenCoreAsync(Registry.Get<TRoute>(), null, null);
        }
        ...
        {
            return OpenCoreAsync(Registry.Get<TRoute>(), args, null);
        }
        ...
        {
            EnsureInitialized();
            UIWindowDefinition definition = Registry.Get<TRoute>();
        ...
        {
            EnsureInitialized();
            UIWindowDefinition definition = Registry.Get(routeName);
            string group = definition.NavigationGroup;
            navigationGroups.TryGetValue(group, out UIWindowHandle previous);
            UIWindowHandle next = await OpenCoreAsync(definition, null, null);
            navigationGroups[group] = next;
            ...
            where TArgs : IUIWindowArgs<TRoute>
        {
            ...
            await OpenCoreAsync(Registry.Get<TRoute>(), args, channel);
        ...
        private void DestroyAllCachedViews()
        {
            ...
            {
                if (!Registry.TryGet(pair.Key, out UIWindowDefinition definition))
                {
                    continue;
                }
        ...
        /// <summary>
        /// 获取当前业务启动周期已经注入的窗口注册表。
        /// </summary>
        /// <returns>窗口注册表。</returns>
        private UIWindowRegistry Registry => registry
            ?? throw new InvalidOperationException("UIService 尚未取得项目窗口注册表。");
