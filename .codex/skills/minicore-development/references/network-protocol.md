...
```text
Proto/Control/                    # 固定 AOT 控制面 Proto
Proto/Business/                   # 可热更新业务 Proto
Proto/Manifest/                   # 两类协议共用 OpcodeManifest
Assets/Scripts/MiniCore/
├── Protocol/Control/             # AOT Control / Control.Inner 生成代码
├── Protocol/Generated/           # HybridCLR Common / Outer / Inner 业务协议
├── Serialization/                # Protobuf 默认序列化与 JSON 对比实现
├── Network/                      # NetworkService、Session、Handler 基类、传输
└── HotUpdate/                    # Client/Server 业务 AMHandler / ARpcHandler
```
...
- 固定 Coordinator 契约放 `Proto/Control`，MiniBomber/NetworkLab/Match/Database 等业务契约放 `Proto/Business`；只有带网络角色标记的消息进入网络注册。生成后不得手改两类 Generated 目录。
...
- AOT `MiniCore.Protocol.Control/Control.Inner` 与 HybridCLR `MiniCore.Protocol.Common/Outer/Inner` 只保存 PB、角色 partial 和无状态消息登记；消息角色契约、Registry 与 Handler 基类属于 `MiniCore.Network`，Parser 适配属于 `MiniCore.Serialization`。
